...
class OtherServiceServiceTest extends TestCase
{
    ...
    {
        $entry = $this->service->create($this->defaultData(['name' => 'Service A']));
        $this->assertEquals('Service A', $entry->name);
    }
    ...
    {
        $this->service->create($this->defaultData(['name' => 'My Service']));
        $this->service->create($this->defaultData(['name' => 'Other']));
        $result = $this->service->list(['search' => 'My Service']);
        $this->assertCount(1, $result->items());
    }
    ...
    {
        for ($i = 0; $i < 110; $i++) {
            $this->service->create($this->defaultData(['name' => "OS{$i}"]));
        }
