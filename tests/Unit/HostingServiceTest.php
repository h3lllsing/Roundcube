...
class HostingServiceTest extends TestCase
{
    ...
    public function test_create(): void
    {
        ...
        $entry = $this->service->create(['user_id' => $user->id, 'name' => 'My Site', 'expiry_date' => '2027-01-01']);
        $this->assertEquals('My Site', $entry->name);
    }
    ...
    {
        $user = User::factory()->create();
        $this->service->create(['user_id' => $user->id, 'name' => 'My Site', 'provider' => 'AWS', 'domain' => 'example.com', 'expiry_date' => '2027-01-01']);
        $this->service->create(['user_id' => $user->id, 'name' => 'Other', 'provider' => 'GCP', 'domain' => 'test.org', 'expiry_date' => '2027-01-01']);
        $result = $this->service->list(['search' => 'My Site']);
        $this->assertCount(1, $result->items());
    }
    ...
    public function test_list_clamps_per_page(): void
    {
        $user = User::factory()->create();
        for ($i = 0; $i < 110; $i++) {
            $this->service->create(['user_id' => $user->id, 'name' => "H{$i}", 'expiry_date' => '2027-01-01']);
        }
