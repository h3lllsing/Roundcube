...
class DomainEmailController extends Controller
{
    ...
    #[OA\Get(
        ...
        parameters: [
            ...
            new OA\Parameter(name: 'sort_by', in: 'query', required: false, schema: new OA\Schema(type: 'string', enum: ['email', 'provider', 'cost', 'status', 'expiry_date', 'created_at'])),
    ...
    public function store(StoreDomainEmailRequest $request): \Illuminate\Http\JsonResponse
    {
    ...
    public function show(Request $request, DomainEmail $domainEmail): \Illuminate\Http\JsonResponse
    {
    ...
    public function update(UpdateDomainEmailRequest $request, DomainEmail $domainEmail): \Illuminate\Http\JsonResponse
    {
        $user = $request->user();
        if (!$user->hasRole('super-admin') && $domainEmail->user_id !== $user->id) {
            abort(403, 'Forbidden');
        }
        return $this->success($this->domainEmailService->update($domainEmail, $request->validated()), 'Domain email updated');
    }
    ...
    public function destroy(Request $request, DomainEmail $domainEmail): \Illuminate\Http\JsonResponse
    {
        $user = $request->user();
        if (!$user->hasRole('super-admin') && $domainEmail->user_id !== $user->id) {
            abort(403, 'Forbidden');
