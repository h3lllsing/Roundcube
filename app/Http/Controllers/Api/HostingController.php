...
class HostingController extends Controller
{
    ...
    public function store(StoreHostingRequest $request): \Illuminate\Http\JsonResponse
    {
    ...
    public function show(Request $request, Hosting $hosting): \Illuminate\Http\JsonResponse
    {
    ...
    public function update(UpdateHostingRequest $request, Hosting $hosting): \Illuminate\Http\JsonResponse
    {
        $user = $request->user();
        if (!$user->hasRole('super-admin') && $hosting->user_id !== $user->id) {
            abort(403, 'Forbidden');
        }
        return $this->success($this->hostingService->update($hosting, $request->validated()), 'Hosting updated');
    }
    ...
    public function destroy(Request $request, Hosting $hosting): \Illuminate\Http\JsonResponse
    {
        $user = $request->user();
        if (!$user->hasRole('super-admin') && $hosting->user_id !== $user->id) {
            abort(403, 'Forbidden');
