...
class OtherServiceService
{
     ...
     * @return \Illuminate\Pagination\LengthAwarePaginator<int, OtherService>
     */
    public function list(array $filters = []): \Illuminate\Pagination\LengthAwarePaginator
    {
        ...
        $sortBy = in_array($filters['sort_by'] ?? 'expiry_date', ['name','service_type','provider','cost','status','created_at','expiry_date']) ? ($filters['sort_by'] ?? 'expiry_date') : 'expiry_date';
    ...
    public function delete(OtherService $entry): void { $entry->delete(); }
}