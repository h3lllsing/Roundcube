...
class VpsService
{
     ...
     * @return \Illuminate\Pagination\LengthAwarePaginator<int, Vps>
     */
    public function list(array $filters = []): \Illuminate\Pagination\LengthAwarePaginator
    {
        ...
        $sortBy = in_array($filters['sort_by'] ?? 'expiry_date', ['name','provider','plan','ip_address','os','cost','status','created_at','expiry_date']) ? ($filters['sort_by'] ?? 'expiry_date') : 'expiry_date';
    ...
    public function delete(Vps $entry): void { $entry->delete(); }
}