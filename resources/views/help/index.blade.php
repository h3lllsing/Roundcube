    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <aside class="lg:col-span-1">
            <nav class="lg:sticky lg:top-24 space-y-0.5 help-sidenav" aria-label="Help navigation">
                    @foreach($navigation as $catKey => $category)
                    <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-2 px-3 mt-4 first:mt-0">{{ $category['label'] }}</p>
                    ...
                    <a href="#"
                        ...
                        data-slug="{{ $doc['slug'] }}"
                        @click.prevent="openDoc('{{ $doc['slug'] }}')"
                        :class="activeDoc === '{{ $doc['slug'] }}' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 font-semibold' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-200'">
