@extends('layouts.admin')

@section('title', 'G-Mails')

@section('content')
<div class="max-w-7xl mx-auto">
    <x-page-header title="G-Mails" subtitle="Manage G-Mail accounts.">
        <x-slot:actions>
            @if($canExport)
            <x-button href="{{ route('export', 'g-mails') }}" variant="success" size="sm">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                Export CSV
            </x-button>
            @endif
            @if($canCreate)
            <x-button href="{{ route('g-mails.create') }}" variant="primary" size="sm">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                Create
            </x-button>
            @endif
        </x-slot:actions>
    </x-page-header>

    <form method="GET" class="flex flex-wrap gap-3 mb-6">
        <x-filter-input name="search" value="{{ request('search') }}" placeholder="Search g-mails..." />
        <x-filter-select name="status" placeholder="All statuses" :options="['active' => 'Active', 'inactive' => 'Inactive', 'suspended' => 'Suspended']" />
        <x-button type="submit" variant="primary" size="sm">Filter</x-button>
        @if(request()->anyFilled(['search', 'status']))
            <x-button href="{{ route('g-mails.index') }}" variant="outline" size="sm">Clear</x-button>
        @endif
    </form>

    <form method="POST" action="{{ route('bulk-action') }}" class="mb-6" id="bulk-form">
        @csrf
        <input type="hidden" name="type" value="g-mails">
        <x-bulk-actions type="g-mails" colspan="10" :actions="$bulkActions" />
    </form>

        <x-table>
            <x-slot:head>
                <th scope="col" class="text-left px-6 py-3 font-medium text-gray-500 dark:text-gray-400">S.No.</th>
                <th scope="col" class="text-left px-6 py-3 font-medium text-gray-500 dark:text-gray-400">Status</th>
                <th scope="col" class="text-left px-6 py-3 font-medium text-gray-500 dark:text-gray-400">User Name</th>
                <th scope="col" class="text-left px-6 py-3 font-medium text-gray-500 dark:text-gray-400">PSEUDO</th>
                <th scope="col" class="text-left px-6 py-3 font-medium text-gray-500 dark:text-gray-400">EMAILS ADDRESS</th>
                <th scope="col" class="text-left px-6 py-3 font-medium text-gray-500 dark:text-gray-400">Password</th>
                <th scope="col" class="text-left px-6 py-3 font-medium text-gray-500 dark:text-gray-400">Department</th>
                <th scope="col" class="text-left px-6 py-3 font-medium text-gray-500 dark:text-gray-400">ASSIGNED</th>
                <th scope="col" class="text-left px-6 py-3 font-medium text-gray-500 dark:text-gray-400">Actions</th>
            </x-slot:head>
                    @forelse ($gMails as $gMail)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            <td class="px-4 py-3"><input type="checkbox" name="ids[]" value="{{ $gMail->id }}" aria-label="Select {{ $gMail->id }}" class="rounded border-gray-300 dark:border-gray-600 text-indigo-600 focus:ring-indigo-500 bulk-item" form="bulk-form"></td>
                            <td class="px-6 py-3 text-gray-500">{{ $gMail->id }}</td>
                            <td class="px-6 py-3"><x-badge :variant="$gMail->status ?? 'inactive'">{{ ucfirst($gMail->status ?? 'inactive') }}</x-badge></td>
                            <td class="px-6 py-3 font-medium">{{ $gMail->user_name ?? '—' }}</td>
                            <td class="px-6 py-3 text-gray-500">{{ $gMail->pseudo ?? '—' }}</td>
                            <td class="px-6 py-3">
                                <div class="flex items-center gap-2">
                                    <span class="font-mono text-gray-500">{{ $gMail->emails_address ?? '—' }}</span>
                                    @if($gMail->emails_address)
                                    <x-copy-button :text="$gMail->emails_address" title="Copy email" />
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-3">
                                @if($gMail->password)
                                    <div class="flex items-center gap-2">
                                        <span class="font-mono text-sm text-gray-400">••••••••</span>
                                        <x-permission-check :module="$gMail->module ?? $vaultModule" action="reveal">
                                        <x-copy-button password-route="{{ route('g-mails.password', $gMail->id) }}" title="Copy password" />
                                        </x-permission-check>
                                    </div>
                                @else
                                    <span class="text-gray-400">—</span>
                                @endif
                            </td>
                            <td class="px-6 py-3 text-gray-500">{{ $gMail->department ?? '—' }}</td>
                            <td class="px-6 py-3 text-gray-500">{{ $gMail->assigned ?? '—' }}</td>
                            <td class="px-6 py-3 whitespace-nowrap">
                            <x-action href="{{ route('g-mails.show', $gMail->id) }}" color="indigo" icon="view" label="" title="View" />
                            <x-permission-check :module="$gMail->module" action="update">
                            <x-action href="{{ route('g-mails.edit', $gMail->id) }}" color="amber" icon="edit" label="" title="Edit" />
                            </x-permission-check>
                            <x-permission-check :module="$gMail->module" action="delete">
                            <x-action action="{{ route('g-mails.destroy', $gMail->id) }}" color="red" icon="delete" label="" title="Delete" confirm="Are you sure?" method="DELETE" />
                            </x-permission-check>
                            </td>
                        </tr>
                @empty
                    <tr><x-empty-state :colspan="11" icon="mail" title="No G-Mails found." message="Add G-Mail accounts to manage them." /></tr>
                @endforelse
            </tbody>
        </x-table>

        <div class="mt-4">{{ $gMails->links() }}</div>
</div>


@endsection
