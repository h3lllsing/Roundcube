...
<div class="max-w-2xl mx-auto">
    <div class="bg-white dark:bg-black rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <form action="{{ route('tasks.update', $task->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PUT')
            <input type="hidden" name="updated_at" value="{{ $task->updated_at->format('Y-m-d H:i:s') }}">
            ...
            <div>
                ...
                <select name="assignee_ids[]" id="assignee_ids" multiple
                    ...
                    @foreach ($users as $key => $label)
                        <option value="{{ $key }}" {{ collect(old('assignee_ids', $task->assignees->pluck('id')->toArray()))->contains($key) ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
