...
<div class="max-w-7xl mx-auto">
    <x-page-header title="Modules" subtitle="Manage modules within features.">
        <x-slot:actions>
            @if(auth()->user()->hasRole('super-admin'))
            <x-button href="{{ route('export', 'modules') }}" variant="success" size="sm">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                Export CSV
            </x-button>
            @endif
            <x-button href="{{ route('modules.create') }}" variant="primary" size="sm">
    ...
    <form method="GET" class="flex flex-wrap gap-3 mb-6">
        ...
        @if(request()->anyFilled(['search', 'status', 'trashed']))
            <x-button href="{{ route('modules.index') }}" variant="outline" size="sm">Clear</x-button>
        @endif
    </form>
    ...
    <form method="POST" action="{{ route('bulk-action') }}" class="mb-6" id="bulk-form">
        ...
        <x-bulk-actions type="modules" colspan="6" :statuses="[]" :actions="['delete', 'restore', 'force-delete']" />
    </form>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-black/50">
                    ...
                    <th scope="col" class="text-left px-6 py-3 font-medium text-gray-500 dark:text-gray-400">Feature</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($modules as $module)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td class="px-4 py-3"><input type="checkbox" name="ids[]" value="{{ $module->id }}" aria-label="Select {{ $module->name }}" class="rounded border-gray-300 dark:border-gray-600 text-indigo-600 focus:ring-indigo-500 bulk-item" form="bulk-form"></td>
                        ...
                        <td class="px-6 py-3 whitespace-nowrap">
                            <x-action href="{{ route('modules.show', $module->id) }}" color="indigo" icon="view" label="View" />
                            <x-action href="{{ route('modules.edit', $module->id) }}" color="amber" icon="edit" label="Edit" />
                            <x-action action="{{ route('modules.destroy', $module->id) }}" color="red" icon="delete" label="Delete" confirm="Are you sure?" method="DELETE" />
                        </td>
                    </tr>
                @empty
                    <tr><x-empty-state :colspan="6" icon="box" title="No modules found." message="Create modules to organize features." /></tr>
                @endforelse
            </tbody>
        </table>
    </div>
