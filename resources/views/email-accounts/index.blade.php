...
<div class="max-w-7xl mx-auto">
    <x-page-header title="Email Accounts" subtitle="Manage email accounts.">
        <x-slot:actions>
            @can('create', App\Models\EmailAccount::class)
                <x-button href="{{ route('email-accounts.create') }}" variant="primary" size="sm">
    ...
    <div class="flex gap-2 mb-4">
        ...
        <a href="{{ route('email-accounts.index', ['trashed' => 1]) }}"
    ...
    <form method="GET" class="flex flex-wrap gap-3 mb-6">
        ...
        @if(request()->anyFilled(['search', 'domain_id', 'status']))
            <x-button href="{{ request('trashed') ? route('email-accounts.index', ['trashed' => 1]) : route('email-accounts.index') }}" variant="outline" size="sm">Clear</x-button>
        @endif
    </form>
                ...
                @forelse ($accounts as $account)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td class="px-6 py-3 font-medium">
                            <a href="{{ route('email-accounts.show', $account) }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ $account->email }}</a>
                        </td>
                            ...
                            @else
                                <div x-data="{ open: false, style: '' }" @click.away="open = false" class="relative inline-block">
                                    ...
                                    <div x-show="open" :style="style" x-cloak role="menu" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="bg-gray-50 dark:bg-black rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 py-1 w-48">
                                        @can('update', $account)
                                            <a href="{{ route('email-accounts.edit', $account) }}" class="block px-3 py-2 text-sm text-gray-700 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" role="menuitem">Edit</a>
                                        @endcan
                                        @can('delete', $account)
                                            <form method="POST" action="{{ route('email-accounts.destroy', $account) }}" class="block">
                                                @csrf
                                                @method('DELETE')
